...
async function main() {
  ...
  if (args.length < 2) {
    console.error(
      'Usage: node diff-checker.js <current-sbom.json> <previous-sbom.json> [output.json]'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!currentSbom.bomFormat || currentSbom.bomFormat !== 'CycloneDX') {
      throw new Error(
        `Invalid current SBOM format. Expected 'CycloneDX', got: '${currentSbom.bomFormat}'`
      );
    }
    if (!previousSbom.bomFormat || previousSbom.bomFormat !== 'CycloneDX') {
      throw new Error(
        `Invalid previous SBOM format. Expected 'CycloneDX', got: '${previousSbom.bomFormat}'`
      );
    }
    ...
    if (!Array.isArray(currentSbom.components)) {
      throw new Error(
        `Invalid current SBOM: 'components' must be an array. Got: ${typeof currentSbom.components}`
      );
    }
    if (!Array.isArray(previousSbom.components)) {
      throw new Error(
        `Invalid previous SBOM: 'components' must be an array. Got: ${typeof previousSbom.components}`
      );
    }
